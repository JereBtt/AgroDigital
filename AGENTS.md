...
- Al registrar un lote cuando existe una campaña activa de la empresa, se solicita una decisión antes de persistir: cancelar, registrar y deshabilitar el lote, o registrarlo y abrir Editar campaña para asociarlo. Este popup es la última validación del formulario: solo puede aparecer cuando los datos obligatorios, el polígono cerrado y la superficie calculada sean válidos. La alternativa de deshabilitar se persiste de forma atómica para que el lote no quede activo sin asociación. Al habilitar un lote deshabilitado con una campaña activa, se muestra un popup con Cancelar y Agregar a la campaña; esta segunda opción habilita el lote y abre Editar campaña para asociarlo. Al salir de Editar campaña, los lotes habilitados sin asociación deben agregarse a la campaña o deshabilitarse; esta misma comprobación se aplica al guardar. El botón Guardar cambios queda deshabilitado si no hay modificaciones.
...
- Densidad de Siembra y Cantidad de Semillas muestran separador de miles con punto en el formulario, mientras conservan su valor numérico sin formato para cálculos y persistencia.
...
- Tras cambios medianos o mayores, especialmente si modifican frontend, API, contratos, rutas, persistencia o validaciones, reiniciar tanto el frontend como la API local antes de cerrar la tarea y comprobar que ambos respondan. Esto evita validar una instancia con codigo anterior por hot reload incompleto o archivos bloqueados.
